...
These files are included with the eCOMET package and can be accessed via `system.file()`.
...
If your metadata contains a `mass` column (tissue mass in mg), then `MassNormalization()` adjusts peak areas for differences in sample mass: `normalized = original * mean(mass) / sample_mass`.
...
SIRIUS provides computational annotations including molecular formula predictions and compound class assignments via CANOPUS (NPClassifier and ClassyFire classifications).
...
Chemical similarity scores (e.g., from DreaMS) can be used for chemically-informed heatmap clustering and diversity analyses. Similarities are stored as-is (0–1, higher = more similar); downstream functions convert to distance internally.
